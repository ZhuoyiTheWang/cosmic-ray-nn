...
def lnA_correlation(mass_true, mass_pred, plot_dir, noise):
    ...
    _, mean_mf, std_mf, _ = tools_discrete.get_mean_and_std(mass_true, mass_pred, 48)
    ...
    bin_centers, mean, mean_err, bin_width = tools_discrete.get_mean_and_err(mass_true, diff, 48)
    bin_centers, std, std_err, bin_width = tools_discrete.get_std_and_err(mass_true, diff, 48)
    ...
    ax1.set_xlabel(r"${A}_\mathrm{MC}$")
    ...
    ax2.errorbar(bin_centers, mean, yerr=mean_err, color='grey', fmt=".", linewidth=2.5, zorder=1)
    ax2.errorbar(bin_centers[0], mean[0], yerr=mean_err[0], color='red', fmt=".", linewidth=2.5,
                    label="proton", zorder=1)
    ax2.errorbar(bin_centers[-6], mean[-6], yerr=mean_err[-6], color='blue', fmt=".", linewidth=2.5,
                    label="Iron", zorder=1)
    ...
    ax3.errorbar(bin_centers, std, yerr=std_err, color='grey', fmt=".", linewidth=2.5, zorder=1)
    ax3.errorbar(bin_centers[0], std[0], yerr=std_err[0], color='red', fmt=".", linewidth=2.5,
                    label="proton", zorder=1)
    ax3.errorbar(bin_centers[-6], std[-6], yerr=std_err[-6], color='blue', fmt=".", linewidth=2.5,
                    label="Iron", zorder=1)
    ...
    ax3.set_xlabel(r"${A}_\mathrm{MC}$")
    ax3.set_ylabel(r"$\sigma\left(\Delta\ln{A}\right)_\mathrm{(DNN-MC)}$")

    fig.tight_layout()
    print('done')
    fig.savefig(plot_dir + f"/dnn_results_discrete{noise}")
